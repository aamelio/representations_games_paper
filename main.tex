...
\title{Beyond Social Preferences: Mental Representations in Games\thanks{We thank seminar audiences for helpful comments. The experiments were pre-registered on AsPredicted (\#261370 and \#286187). % TODO: add funding, IRB, and acknowledgment details.
}}
...
\date{Version 2.0 (Skeleton Draft) --- July 2026}
...
\begin{abstract}
\noindent Behavior in economic games shifts with contextual features that leave rules, payoffs, and information unchanged. We propose that context changes how players mentally \emph{represent} a game: which situations it evokes and, as a result, how much attention they pay to their own earnings, to total earnings, and to a norm of equal sharing. We measure representations directly, classifying participants' open-ended explanations of their choices in dictator, ultimatum, and trust games into moral, self-interested, and cooperative categories. Absent any contextual manipulation, representations vary systematically across games and predict actions and beliefs. Randomized, payoff-irrelevant changes in context shift representations and, with them, actions and beliefs; the story-induced shifts track the measured structural similarity between story and game. Representation shifts, combined with how each representation behaves, quantitatively account for treatment effects on actions and beliefs across games and roles. Beliefs display systematic, context-dependent forecast errors, inconsistent with rational-expectations accounts of these effects.
\medskip
...
This section describes the design and measurement (Section~\ref{sec:design_details}), establishes in the control sample the within-context facts the model organizes---representations differ across games, and they predict actions and beliefs (Section~\ref{sec:control})---and measures the similarity structure that links our treatments to the retrieval model (Section~\ref{sec:similarity}).
...
The control sample establishes three facts---representations differ across games, in what they are about and in whom they evoke; they predict actions; and they predict beliefs---and a fourth that ties the last two together: how strongly actions respond to beliefs is itself part of the representation.
...
\paragraph{Representations evoke counterparts.} Representations differ not only in what the situation is about but in \emph{whom} it calls to mind. The classification codes the social proximity of the counterpart mentioned in each answer (Section~\ref{sec:design_details}), and Table~\ref{tab:player1_sp_by_category} cross-tabulates it with the categories: self-interested representations almost never picture a socially close counterpart---self-interested, high-proximity responses are 1.9\% of the pooled classified sample, against 21.9\% for self-interested, low-proximity ones---while moral representations split evenly between close and distant counterparts. Proximity is behaviorally live in its own right, and in the pattern the model implies. In control, dictators whose answer evokes a close counterpart transfer 47.0\% of the budget, against 29.5\% for those who evoke a distant or absent one; the same gap is 2.9 points in the ultimatum game---compressed by rejection risk like every other gap---and 10.1 points in the trust game. Whom one imagines facing is part of the representation, and it will move with the same levers as everything else: the market frame largely removes the counterpart from the description (Section~\ref{sec:market_control}).
...
\input{replication_package/output/tables/player1_sp_by_category.tex}

\paragraph{Representations predict actions.} Within each game, categories order actions exactly as the attention weights imply---and the ordering interacts with strategic structure in the way that is the model's signature. Figure~\ref{fig:control_outcomes} reports mean actions by category. Where nothing disciplines the dictator, the gap is enormous: moral dictators transfer 46.2\% of the budget and self-interested dictators 10.3\%. In the UG the same gap collapses---moral proposers offer 50.0\% and self-interested proposers 41.7\%---exactly the strategic discipline of Proposition~\ref{prop:ug}. In the TG, sends follow the efficiency ranking of Proposition~\ref{prop:tg}: cooperative senders send the most (63.5\%), moral senders 44.1\%, and self-interested senders 29.5\%. The \emph{same} category (Self-interest) thus predicts near-bottom transfers in the DG and near-fair offers in the UG\@.
...
\paragraph{Representations predict beliefs.} The belief data separate two regimes that the model distinguishes: a commonly held standard in the UG, and projection in the TG\@. Figure~\ref{fig:control_beliefs} reports hypothetical beliefs at the one-third reference action. In the UG, beliefs about acceptance are essentially flat across categories (48.2--48.5\%): proposers of every representation hold essentially the same believed acceptance schedule regardless of their own category, consistent with acceptance being governed by a commonly held standard. This shared standard is what compresses offers: under full projection, self-interested proposers would expect low offers to be accepted, and offers would collapse. In the TG, by contrast, beliefs line up with representations exactly as projection implies: self-interested senders expect back 22.0\% of the tripled amount, moral senders 31.5\%, and cooperative senders 35.7\%. Players who represent the game as a cooperative venture also expect cooperation from their counterpart; players who represent it through own payoffs project a trustee who keeps.
...
\paragraph{Actions and beliefs jointly.} The model ties the two elements of the representation together: the action should respond to beliefs---negatively in the UG, positively in the TG (Table~\ref{tab:cs_summary})---and the strength of that response should itself depend on attention, strongest for Self-interest representations---the sensitivity is increasing in $\sigma$ and decreasing in $\mu$ (Propositions~\ref{prop:ug} and~\ref{prop:tg}). Figure~\ref{fig:belief_sensitivity} shows both patterns in the control data. In the UG, moral proposers do not chase beliefs at all (slope $+0.01$): they act on the norm. Self-interested proposers do (slope $-0.11$), offering less the likelier they believe a low offer is to be accepted. In the TG, every category's sends rise with the believed return, and the Self-interest slope is again the steepest ($+0.45$, against $+0.31$ for Moral). Pooling all four arms sharpens the estimates and rejects equality of slopes across categories in both games ($p=0.002$ in the UG, $p<0.001$ in the TG\@; Table~\ref{tab:belief_sensitivity} in Appendix~\ref{app:player2}), with the pooled Mutual Benefit/Cooperation slope in the TG the smallest of the three ($0.13$)---also as the mapping implies, since that category loads on efficiency rather than own payoff. Finally, the UG slopes are an order of magnitude smaller than the TG slopes ($-0.09$ against $+0.47$ for Self-interest in the pooled specification): in the ultimatum game the acceptance schedule itself damps the response---the sensitivity $\sigma/(2b\sigma+\mu)$ is bounded above by $1/(2b)$---while nothing bounds the trust game's $\sigma(1+r)/\mu$. These category-specific slopes are the disaggregation of the pooled belief coefficients in the benchmark regressions of Section~\ref{sec:benchmark}.
...
These control-sample objects are the inputs of everything that follows: by the decomposition of Section~\ref{sec:determination}, a context manipulation moves mean behavior through a retrieval shift---whose size is gated by the similarity we measure next---multiplied by the spread of category-typical actions just documented, widest where nothing disciplines the action.
...
In the language of Section~\ref{sec:determination}, the stories operate the story lever: they raise the retrieval weight $F_c$ of their category while leaving the description of the game untouched. Two features of Figure~\ref{fig:te_aid_bonus} are fingerprints of this lever. First, representations and actions move while beliefs stay put: the boost travels through what the sender attends to, not through whom she imagines facing. Second, the effects fade with the structural distance between the stories and the game measured in Section~\ref{sec:similarity} (Table~\ref{tab:llm_similarity}).

Table~\ref{tab:similarity_gradient} lines the two up, adding DG-LT, the give-or-take dictator game of Figure~\ref{fig:intro_kw_lt} (its story arms appear here and in the dispersion accounting of Section~\ref{sec:heterogeneity}). Story effects on actions decline monotonically in structural similarity---$-5.6$, $-4.7$, $-2.4$, $+0.5$ (n.s.) percentage points as similarity falls from 94.6 to 51.2 to 29.9 to 14.4---and so does the story's effect on the representations themselves: the Moral share falls by 9.7 to 14.5 points in the three structurally close games and by a negligible 0.4 points in the TG\@. In the decomposition of Section~\ref{sec:determination}, $\Delta E[t^{g}]=\sum_{c}\Delta q_{c}\,\bar{t}_{c}^{\,g}$, the Moral-share column shows that the retrieval shift $\Delta q_{c}$ is itself gated by similarity; the action column is its behavioral consequence. Four games make this an ordinal check, not an estimate, but the ranking is exact. It also answers the natural question about the trust game: sends do not move because the retrieval shift vanishes there---the TG is the structurally remotest game---not because the counterpart drops out of the representation; the term $\rho r-\sigma(1+r)(1-s)$ moves every TG category's send (equation~\eqref{eq:tg_send}).
...
\subsection{Changing Context \texorpdfstring{($\boldsymbol{\kappa}_d$)}{(kd)}: Market vs Control}
...
Two further features of these results resist explanations based on stable preferences plus standard channels. First, the same manipulation moves behavior in opposite directions across games (giving falls, trusting rises), which no single-dimensional shift in generosity or selfishness can produce, but which a reallocation of attention across three motives produces naturally. Second, beliefs move together with representations---strongly under the market frame, not at all under the stories. The asymmetry is what the two levers suggest: a story raises the retrieval frequency of a category while the described counterpart is unchanged, so attention moves and beliefs stay anchored; the market frame relocates the description itself, so the retrieved category changes wholesale---attention \emph{and} the imagined counterpart.
...
The disappearing counterpart is directly visible in the texts. Under the market frame, the share of answers that mention no counterpart at all roughly triples in the reasons measure---from 18.5\% to 57.4\% in the DG-KW, 12.9\% to 40.5\% in the UG, and 12.2\% to 38.7\% in the TG---and the hypothetical-allocation measure shows the same disappearance (28.8\% to 75.9\%, Appendix~\ref{app:hp}). Read together with the rise of Mutual Benefit/Cooperation, the trust-game pattern is the signature of \emph{generalized} rather than relational trust: market senders send more to a counterpart they no longer picture as a particular person---the attitude toward unspecified others that the trust literature measures with survey questions and investment games.\footnote{\citet{GlaeserLaibsonScheinkmanSoutter2000} find that survey measures of generalized trust predict trustworthiness in the investment game better than they predict sending; \citet{Cox2004} separates trust from unconditional generosity in sending; \citet{AshrafBohnetPiankov2006} decompose trust into expectations and kindness; \citet{JohnsonMislin2011} meta-analyze 162 trust-game replications; \citet{Karlan2005} links game behavior to real repayment; \citet{Fehr2009} reviews.} The market frame manipulates anonymity as much as incentives, and where the strategic structure rewards cooperation with abstract counterparties, anonymity does not preclude trust.
...
The economic content of this exercise is the paper's central claim: the same handful of measured representations that organizes who gives, offers, trusts, and rejects \emph{within} a context also accounts for how behavior moves \emph{across} contexts. Heterogeneity and instability are two faces of the same object.
